...
-- Создание функции объединения полей VisitorNaming и VisitingNumber для поиска 

-- changeset liquibase:2 splitStatements:false
create or replace function fullvisiting(VisitorNaming text, VisitingNumber text)
returns text
language plpgsql
immutable
as $$
begin
  return regexp_replace(
    lower(
      trim(
	VisitorNaming::text || ' ' || VisitingNumber::text
      )
    ),
    'ё',
    'е',
...
create or replace function fullvisitingpage(VisitorNaming text, VisitingNumber text)
returns text
language plpgsql
immutable
as $$
